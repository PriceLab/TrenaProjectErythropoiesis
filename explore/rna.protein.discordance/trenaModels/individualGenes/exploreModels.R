library(RUnit)
files <- list.files(pattern="trena.*RData")
...
test_assignRank <- function()
{
   ...
   if(!exists("tbl.test"))
      tbl.test <- get(load("tbl.test.RData"))
   tbl.sub <- subset(tbl.test, target=="OGT")
   dim(tbl.sub)  # 117 16
...
test_assignRankAllTables <- function()
{
   ...
   tbl.ranked <- assignRankAllTables(tbl.test, "spear.pos")
   checkEquals(nrow(tbl.test), nrow(tbl.ranked))
   checkEquals(subset(tbl.ranked, gene=="STAU1" & rank <=10)$target, c("BACH1", "OGT", "RCOR1"))

   tbl.ranked <- assignRankAllTables(tbl.test, "rf")
   x <- as.list(head(sort(table(subset(tbl.ranked, rank <= 5)$gene), decreasing=TRUE), n=3))
   checkEquals(x, list(KLF9=5, SP1=4, FOSL1=3))

} # test_assingRankAllTables
#------------------------------------------------------------------------------------------------------------------------
combine <- function()
{
...
top.regulators <- function()
{
   ...
   tbl.ranked <- assignRankAllTables(tbl.test, "spear.abs")
   head(as.data.frame(sort(table(subset(tbl.ranked, rank <= max)$gene), decreasing=TRUE)), n=10)

   tbl.ranked <- assignRankAllTables(tbl.test, "pear.pos")
   head(as.data.frame(sort(table(subset(tbl.ranked, rank <= max)$gene), decreasing=TRUE)), n=10)

   tbl.ranked <- assignRankAllTables(tbl.test, "pear.abs")
   head(as.data.frame(sort(table(subset(tbl.ranked, rank <= max)$gene), decreasing=TRUE)), n=10)

   tbl.ranked <- assignRankAllTables(tbl.test, "lasso.abs")
   head(as.data.frame(sort(table(subset(tbl.ranked, rank <= max)$gene), decreasing=TRUE)), n=10)

   tbl.ranked <- assignRankAllTables(tbl.test, "lasso.pos")
   head(as.data.frame(sort(table(subset(tbl.ranked, rank <= max)$gene), decreasing=TRUE)), n=10)
