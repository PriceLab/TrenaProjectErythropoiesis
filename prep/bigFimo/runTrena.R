...
build.models.with.inclusiveFimoTable <- function()
{
   ...
   tfs <- unique(subset(tbl.tms, fimo_pvalue < 1e-3 & abs(cor.all) > 0.3 & (gh > 10 | chip | oc))$tf)
   ...
   tbl.trena.tf$rank <- seq_len(nrow(tbl.trena.tf))
   
   tbl.trena.both$target <- targetGene
   tbl.trena.both$rank <- seq_len(nrow(tbl.trena.both))

   print(head(tbl.trena.tf, n=10))
   print(head(tbl.trena.both, n=10))
